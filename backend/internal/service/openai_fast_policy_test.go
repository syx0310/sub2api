...
func TestApplyOpenAIFastPolicyToBody_GroupForceLeavesUnsupportedModelsStandard(t *testing.T) {
	svc := newOpenAIGatewayServiceWithSettings(t, DefaultOpenAIFastPolicySettings())
	account := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth}
	ctx := context.WithValue(context.Background(), ctxkey.Group, &Group{
		ID: 7, Platform: PlatformOpenAI, Status: StatusActive, Hydrated: true, ForceOpenAIFast: true,
	...
	for _, model := range []string{"gpt-5.4-mini", "gpt-5.4-nano", "gpt-5.5-pro", "gpt-5.3-codex-spark"} {
		body := []byte(`{"model":"` + model + `","input":"hi"}`)
		updated, err := svc.applyOpenAIFastPolicyToBody(ctx, account, model, body)
		require.NoError(t, err)
		require.False(t, gjson.GetBytes(updated, "service_tier").Exists(), model)
	}
}
