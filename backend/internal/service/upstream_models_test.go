...
func TestSyncUpstreamModelCatalogUsesConfiguredModelsWhenListEndpointUnsupported(t *testing.T) {
	...
	lastLiveSync := time.Now().UTC().Add(-upstreamModelCatalogRoutingTTL - time.Minute).Format(time.RFC3339)
	account.SetUpstreamModelMetadataSnapshot(UpstreamModelMetadataSnapshot{
		ModelIDsSyncedAt: lastLiveSync,
		ModelIDs:         []string{"old-live-model"},
	...
	require.Equal(t, lastLiveSync, account.GetUpstreamModelMetadataSnapshot().ModelIDsSyncedAt)
	require.Equal(t, []string{"old-live-model"}, account.GetUpstreamModelMetadataSnapshot().ModelIDs)
	_, known := account.UpstreamModelCatalogSupports("gpt-6-astra", time.Now())
	require.False(t, known, "capability refresh must not renew expired routing evidence")
...
func TestUpstreamModelCatalogSupportsFreshTypedAndHydratedSnapshots(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			raw: UpstreamModelMetadataSnapshot{
				ModelIDsSyncedAt: now.Add(-time.Hour).Format(time.RFC3339),
		...
		},
		{
			...
			raw: map[string]any{
				"model_ids_synced_at": now.Add(-time.Hour).Format(time.RFC3339),
	...
	stale := &Account{Extra: map[string]any{UpstreamModelMetadataExtraKey: UpstreamModelMetadataSnapshot{
		ModelIDsSyncedAt: now.Add(-upstreamModelCatalogRoutingTTL - time.Second).Format(time.RFC3339),
...
func TestIsModelSupportedUsesFreshCatalogOnlyForGPT6AstraRollout(t *testing.T) {
	...
	account.SetUpstreamModelMetadataSnapshot(UpstreamModelMetadataSnapshot{
		ModelIDsSyncedAt: now.Add(-upstreamModelCatalogRoutingTTL - time.Minute).Format(time.RFC3339),
		ModelIDs:         []string{"gpt-5.6-sol"},
	})
