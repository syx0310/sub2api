...
func TestApplyCodexOAuthTransform_NormalizesNativeToolCallPairsByType(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, "fc_custom", custom["id"], "Codex accepts any non-empty prefixed item id")
	require.Equal(t, "ctc_custom", custom["call_id"])
	require.Equal(t, custom["call_id"], customOutput["call_id"])
	require.Equal(t, "fc_search", search["id"], "the item-id prefix is not tied to the concrete item type")
...
func TestApplyCodexOAuthTransform_GPT6AstraKeepsNativeInstructionsPlaceholder(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-6-astra",
	}
	...
	instructions, ok := reqBody["instructions"].(string)
	require.True(t, ok)
	require.Empty(t, instructions, "native Codex requests must not receive a synthesized system prompt")
	require.True(t, result.Modified)
}
...
func TestRewriteSystemToDeveloper(t *testing.T) {
	t.Run("no system messages", func(t *testing.T) {
		...
		require.True(t, ok)
		require.Len(t, input, 1)
		result := rewriteSystemToDeveloper(input)
	...
	t.Run("existing instructions preserved", func(t *testing.T) {
		reqBody := map[string]any{
			"input": []any{
...
func TestExtractSystemMessagesFromInput(t *testing.T) {
	...
	t.Run("mixed system and non-system preserves non-system", func(t *testing.T) {
		...
		require.True(t, ok)
		require.Len(t, input, 3)
		system, ok := input[1].(map[string]any)
		require.True(t, ok)
		require.Equal(t, "developer", system["role"])
	})
...
func TestApplyCodexOAuthTransform_ResponsesLiteRewritesSystemWithoutInstructions(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.4",
		"input": []any{
			map[string]any{"role": "system", "content": "Keep this in input."},
			map[string]any{"role": "user", "content": "hello"},
		},
	}
	...
	require.True(t, ok)
	require.Equal(t, "developer", developer["role"])
	require.Equal(t, "Keep this in input.", developer["content"])
}
