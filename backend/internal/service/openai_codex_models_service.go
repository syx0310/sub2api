...
// BuildGroupConfiguredCodexModelsManifest builds a Codex catalog from
// authoritative account mappings, with fallback models for unmapped accounts
// in the same OpenAI group. The boolean result distinguishes "no authoritative
// configuration" from a configured catalog that becomes empty after group-level filtering. System-
...
func (s *OpenAIGatewayService) BuildGroupConfiguredCodexModelsManifest(
	...
	configuredModels := supplementUnmappedOpenAIModels(visible, openAIConfiguredAndObservedCodexModelIDsForGroup(visible, group))
...
// Codex model entries. The group model allowlist also filters the picker,
// matching the standard /v1/models listing and request admission policy.
...
// 112be0bd74. OAuth manifests still come from
...
func mergeConfiguredCodexModelsManifest(
	...
	for _, rawModel := range upstreamModels {
		...
		if isCodexSparkModel(descriptor.Slug) {
			textOnlyModel, capabilityChanged, err := codexModelWithTextOnlyInput(rawModel)
			if err != nil {
				return nil, false, err
			}
			rawModel = textOnlyModel
			changed = changed || capabilityChanged
		}
...
func codexModelWithTextOnlyInput(rawModel json.RawMessage) (json.RawMessage, bool, error) {
	var fields map[string]json.RawMessage
	if err := json.Unmarshal(rawModel, &fields); err != nil {
		return nil, false, err
	}
	if !enforceCodexTextOnlyInput(fields) {
		return rawModel, false, nil
	}
	updated, err := json.Marshal(fields)
	if err != nil {
...
func enforceCodexTextOnlyInput(fields map[string]json.RawMessage) bool {
	if fields == nil {
		return false
	}
	changed := false
	modalities := json.RawMessage(`["text"]`)
	if !bytes.Equal(bytes.TrimSpace(fields["input_modalities"]), modalities) {
		fields["input_modalities"] = modalities
		changed = true
	}
	imageDetailOriginal := json.RawMessage("false")
	if !bytes.Equal(bytes.TrimSpace(fields["supports_image_detail_original"]), imageDetailOriginal) {
		fields["supports_image_detail_original"] = imageDetailOriginal
		changed = true
	}
	return changed
}
...
func applySyncedAPIKeyCodexModelMetadata(body []byte, account *Account, overwriteLocalDefaults bool) ([]byte, error) {
	...
	for i, rawModel := range models {
		...
		descriptor := newConfiguredCodexModelDescriptor(lookupModel)
...
func completeAPIKeyCodexModelsManifestMetadata(body []byte, completeAll bool, account *Account) ([]byte, error) {
	...
	for i, rawModel := range models {
		...
		completeDescriptor := completeAll || isDeepSeekCodexModel(capabilityModel)
		forceOfficialImage := officialOpenAI && isOpenAICodexImageInputModel(capabilityModel)
		forceSparkText := isCodexSparkModel(capabilityModel)
		if !completeDescriptor && !forceOfficialImage && !forceSparkText {
			continue
		}

		descriptor := newConfiguredCodexModelDescriptor(capabilityModel)
		descriptor.Slug = slug
		if capabilityModel != slug {
		...
		if forceSparkText && enforceCodexTextOnlyInput(model) {
			modelChanged = true
		}
		if !modelChanged {
			continue
