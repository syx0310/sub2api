...
func TestFilterCodexInput_PreservesWellFormedCrossNamespaceToolIDs(t *testing.T) {
	input := []any{
	...
	require.Equal(t, "fc_wrong", filtered[0].(map[string]any)["id"])
	require.Equal(t, "fc_wrong", filtered[1].(map[string]any)["id"])
}
