...
func TestCodexRequestBodyIdentityNamespaceIsStablePerOAuthAccount(t *testing.T) {
	body := []byte(`{"model":"gpt-5.6-codex","prompt_cache_key":"client-session","client_metadata":{"x-codex-installation-id":"client-installation","session_id":"client-session","thread_id":"client-thread","x-codex-parent-thread-id":"client-parent-thread","turn_id":"client-turn","parent_turn_id":"client-parent-turn","root_turn_id":"client-root-turn","x-codex-window-id":"client-window","x-codex-turn-metadata":"{\"installation_id\":\"client-installation\",\"session_id\":\"client-session\",\"thread_id\":\"client-thread\",\"forked_from_thread_id\":\"client-fork-thread\",\"parent_thread_id\":\"client-parent-thread\",\"turn_id\":\"client-turn\",\"parent_turn_id\":\"client-parent-turn\",\"root_turn_id\":\"client-root-turn\",\"window_id\":\"client-window\"}"}}`)
	...
	require.Equal(t, gjson.GetBytes(first, "client_metadata.x-codex-parent-thread-id").String(), embeddedFirst["parent_thread_id"])
	require.Equal(t, gjson.GetBytes(first, "client_metadata.parent_turn_id").String(), embeddedFirst["parent_turn_id"])
	require.Equal(t, gjson.GetBytes(first, "client_metadata.root_turn_id").String(), embeddedFirst["root_turn_id"])
}
...
func TestBuildOpenAIWSHeadersNamespacesCodexIdentityByOAuthAccount(t *testing.T) {
	...
	c.Request.Header.Set("x-codex-parent-thread-id", "client-parent-thread")
	...
	for _, header := range []string{"session_id", "x-codex-installation-id", "thread-id", "x-codex-parent-thread-id", "x-codex-window-id", "x-client-request-id"} {
