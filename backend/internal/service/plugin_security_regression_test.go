...
func TestPluginRPCAmbiguityPreventsReplayAfterMetadataDelivery(t *testing.T) {
	...
	result := (&OpenAIGatewayService{cfg: &config.Config{Gateway: config.GatewayConfig{
		OpenAITransportErrorFailoverEnabled: true,
	}}}).handleOpenAIUpstreamTransportError(context.Background(), c, account, err, true)
