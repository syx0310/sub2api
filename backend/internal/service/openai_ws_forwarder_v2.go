...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	if payloadTurnMetadata != "" {
		...
		identityHeaders.Set(openAIWSTurnMetadataHeader, payloadTurnMetadata)
		applyCodexAccountIdentityHeaders(identityHeaders, codexAccountIdentitySource(c, account), getAPIKeyIDFromContext(c))
		payloadTurnMetadata = identityHeaders.Get(openAIWSTurnMetadataHeader)
	}
	setOpenAIWSTurnMetadata(payload, payloadTurnMetadata)
