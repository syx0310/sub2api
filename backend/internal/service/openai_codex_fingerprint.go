...
const (
	...
	// codexFingerprintDevice 仅收敛 installation_id 为账号级恒定值（默认行为）。
	// 上游看到 1 台设备 + 多会话（每用户各自的 session）。
	codexFingerprintDevice codexFingerprintMode = "device"
	// codexFingerprintSession 收敛 installation_id + session_id，
	// thread/turn/window 及父子关系标识仍由客户端持有并原样透传。
	// 上游看到 1 台设备 + 1 会话 + N 个真实客户端线程。
...
// 未设置或值无效时默认 device（仅设备收敛），显式设为 "off" 才关闭。
...
// extractClientSessionID preserves the client's original session identity for
// turn-state correlation. Fingerprint session mode does not use it to derive or
// rewrite deeper thread/turn/window identifiers.
...
// clientSessionID 为兼容既有调用保留；只有 full 模式会生成深层标识，session
// 模式不会派生或改写客户端 thread/turn/window。调用方必须共享同一个 IDs 实例。
func resolveCodexFingerprintIDs(account *Account, _ string, mode codexFingerprintMode) *codexFingerprintIDs {
...
// resolveCodexFingerprintIDsFromRequest 按账号配置解析收敛 ID 集合。
// 调用方应将返回的 ids 同时传给
...
func applyCodexFingerprintHeaders(h http.Header, ids *codexFingerprintIDs) {
	...
	// session 模式只统一设备和会话，保留客户端 thread/turn/window 拓扑。
