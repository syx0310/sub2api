...
func (m *openAIWSPassthroughUsageMeta) initFromFirstFrame(policyOutput []byte, mappedModel string) {
	...
	m.reasoningEffort.Store(extractOpenAIReasoningEffortFromBody(policyOutput, m.sessionRequestModel, mappedModel))
...
func (m *openAIWSPassthroughUsageMeta) updateFromResponseCreate(policyOutput []byte, mappedModel string, requestModelForFrame string) {
	...
	m.serviceTier.Store(extractOpenAIServiceTierFromBody(policyOutput))
	m.reasoningEffort.Store(extractOpenAIReasoningEffortFromBody(policyOutput, requestModelForFrame, mappedModel))
...
func (c *openAIWSPassthroughFirstOutputFrameConn) WriteFrame(ctx context.Context, msgType coderws.MessageType, payload []byte) error {
	...
	if msgType == coderws.MessageText && strings.TrimSpace(gjson.GetBytes(payload, "type").String()) == "response.create" {
		c.armDeadline(payload)
	}
	if err := c.inner.WriteFrame(ctx, msgType, payload); err != nil {
		// A failed flush does not prove the peer rejected the frame. Keep the
		// response deadline armed so the relay can drain a possibly accepted turn.
		return err
	}
	return nil
}

func (c *openAIWSPassthroughFirstOutputFrameConn) remainingDeadline() time.Duration {
	state := c.deadlineState()
	if !state.armed || state.deadline.timeout <= 0 {
		return 0
	}
...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	activeRequestBodyBytes := initialRequestBodyBytes
	sealedRequestBodyBytes := int64(0)
	hasSealedRequestBodyBytes := false
	interTurnRequestBodyBytes := int64(0)
	requestBodyBytesActive := true
	pendingRequestBodyBytes := atomic.Int64{}
	pendingRequestBodyBytes.Store(1)
	var requestBodyBytesMu sync.Mutex
	var turnHooksMu sync.Mutex
	pushRequestBodyBytes := func(payloadBytes int64) {
		requestBodyBytesMu.Lock()
		defer requestBodyBytesMu.Unlock()
		activeRequestBodyBytes = interTurnRequestBodyBytes + payloadBytes
		interTurnRequestBodyBytes = 0
		requestBodyBytesActive = true
		pendingRequestBodyBytes.Add(1)
	}
	addRequestBodyBytes := func(payloadBytes int64) {
		requestBodyBytesMu.Lock()
		defer requestBodyBytesMu.Unlock()
		if requestBodyBytesActive {
			activeRequestBodyBytes += payloadBytes
			return
		}
		interTurnRequestBodyBytes += payloadBytes
	}
	sealRequestBodyBytes := func() {
		requestBodyBytesMu.Lock()
		defer requestBodyBytesMu.Unlock()
		if !requestBodyBytesActive {
			return
		}
		sealedRequestBodyBytes = activeRequestBodyBytes
		hasSealedRequestBodyBytes = true
		activeRequestBodyBytes = 0
		requestBodyBytesActive = false
	}
	popRequestBodyBytes := func() int64 {
		requestBodyBytesMu.Lock()
		defer requestBodyBytesMu.Unlock()
		if !hasSealedRequestBodyBytes {
			return 0
		}
		value := sealedRequestBodyBytes
		sealedRequestBodyBytes = 0
		hasSealedRequestBodyBytes = false
		pendingRequestBodyBytes.Add(-1)
		return value
	}
	...
	if hooks != nil && hooks.BeforeTurn != nil {
		turnHooksMu.Lock()
		beforeTurnErr := hooks.BeforeTurn(1)
		turnHooksMu.Unlock()
		if beforeTurnErr != nil {
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if isResponseCreate {
				...
				if hooks != nil && hooks.BeforeRequest != nil {
					// Prompt Audit sees the client frame before any compatibility,
					// channel, account, or policy rewrite.
			...
			if policyErr == nil && blocked == nil && isResponseCreate {
				...
				if hooks != nil && hooks.BeforeTurn != nil {
					turnHooksMu.Lock()
					beforeTurnErr := hooks.BeforeTurn(turnNo)
					turnHooksMu.Unlock()
					if beforeTurnErr != nil {
						return payload, nil, beforeTurnErr
					}
				}
	...
	if firstWriteErr != nil {
		firstWriteErr = fmt.Errorf("write first upstream websocket request: %w", firstWriteErr)
	}
	...
	relayResult, relayExit := openaiwsv2.RunEntry(openaiwsv2.EntryInput{
		...
		Options: openaiwsv2.RelayOptions{
			...
			IdleTimeout:                     0,
			UpstreamDrainTimeout:            s.openAIWSClientDrainTimeout(),
			FirstMessageType:                coderws.MessageText,
			FirstMessageSent:                true,
			FirstMessageWriteError:          firstWriteErr,
			WriteErrorDrainRemaining:        relayUpstreamFrameConn.remainingDeadline,
	...
	result.ClientDisconnect = IsOpenAIWSClientDeliveryError(turnErr)
	if pendingRequestBodyBytes.Load() > 0 && hooks != nil && hooks.AfterTurn != nil {
		turnHooksMu.Lock()
