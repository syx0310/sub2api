...
func (s *BillingService) initFallbackPricing() {
	...
	s.fallbackPrices[openai.GPT6AstraModelID] = &ModelPricing{
		InputPricePerToken:                 10e-6,
...
func (s *BillingService) getFallbackPricing(model string) *ModelPricing {
	...
	if normalized := normalizeKnownOpenAICodexModel(modelLower); normalized != "" {
		...
		case openai.GPT6AstraModelID:
			return s.fallbackPrices[openai.GPT6AstraModelID]
		case "gpt-5.6-terra":
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	// OpenAI keeps the fork's explicit two-level gate: both the group policy and
	// the selected account must allow context-tier billing. Platforms without an
	// account-level setting pass nil and continue to follow the group policy.
...
func openAIModelFastPricingRatio(normalized string) float64 {
	switch normalized {
	case openai.GPT6AstraModelID, "gpt-5.4", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna":
		return 2.0
	case "gpt-5.5":
		return 2.5
	default:
