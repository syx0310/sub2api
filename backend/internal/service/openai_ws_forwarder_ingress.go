...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(raw []byte) (openAIWSClientPayload, error) {
		...
		responsesLite := isOpenAIResponsesLitePayload(normalized)
		codexBridgeEnabled := isCodexCLI && !responsesLite && imageGenerationAllowed && codexImageGenerationExplicitToolPolicy != codexImageGenerationExplicitToolPolicyStrip && s.isCodexImageGenerationBridgeEnabled(ctx, account, apiKey)
