...
func normalizeOpenAIPassthroughOAuthBody(body []byte, compact, responsesLite bool) ([]byte, bool, error) {
	...
	if input := gjson.GetBytes(normalized, "input"); input.IsArray() {
		...
		input.ForEach(func(_, item gjson.Result) bool {
			...
			next, err := sjson.SetBytes(normalized, fmt.Sprintf("input.%d.role", idx), "developer")
			if err != nil {
				rewriteErr = err
				return false
			}
		...
		if rewriteErr != nil {
			return body, false, fmt.Errorf("normalize passthrough body system role: %w", rewriteErr)
		}
	}
...
func openAIModelSupportsPriorityServiceTier(model string) bool {
	return openAIModelFastPricingRatio(normalizeKnownOpenAICodexModel(model)) > 0
}

func openAIGroupForcesFast(ctx context.Context, account *Account, model string) bool {
...
// ForceOpenAIFast sets service_tier to "priority" for models whose catalog and
// billing definitions both support Fast. Unsupported models remain Standard.
...
func (s *OpenAIGatewayService) applyOpenAIFastPolicyToBody(ctx context.Context, account *Account, model string, body []byte) ([]byte, error) {
	...
	if openAIGroupForcesFast(ctx, account, model) {
...
//   - Group ForceOpenAIFast: sets priority first for Fast-capable models, then
//     applies the global rule
//
