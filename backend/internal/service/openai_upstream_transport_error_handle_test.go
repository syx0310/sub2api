...
func openAITransportErrFailoverEnabledConfig() *config.Config {
	cfg := &config.Config{}
	cfg.Gateway.OpenAITransportErrorFailoverEnabled = true
	return cfg
}

func TestHandleOpenAIUpstreamTransportError_DefaultDisabledWrites502NoFailover(t *testing.T) {
	repo := &openaiTransportAccountRepoStub{}
	svc := &OpenAIGatewayService{accountRepo: repo}
	account := &Account{ID: 4628, Name: "proxy-expired-disabled", Platform: PlatformOpenAI}
	...
	require.False(t, errors.As(err, &fo), "default disabled setting must not return *UpstreamFailoverError")
...
func TestHandleOpenAIUpstreamTransportError_PersistentEvictsAndFailsOver(t *testing.T) {
	repo := &openaiTransportAccountRepoStub{}
	svc := &OpenAIGatewayService{accountRepo: repo, cfg: openAITransportErrFailoverEnabledConfig()}
...
func TestForwardAsRawChatCompletions_TransportErrorFailsOver(t *testing.T) {
	...
	cfg := openAITransportErrFailoverEnabledConfig()
