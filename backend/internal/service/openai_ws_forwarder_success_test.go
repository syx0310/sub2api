...
func TestOpenAIGatewayService_Forward_WSv2_CodexFingerprintHandshakeBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	require.Equal(t, "header-thread", gjson.Get(bodyTurnMetadata, "thread_id").String())
	require.Equal(t, "header-turn", gjson.Get(bodyTurnMetadata, "turn_id").String())
	require.Equal(t, "header-window", gjson.Get(bodyTurnMetadata, "window_id").String())
