...
// Codex replays any ID with a non-empty prefix and suffix separated by an
// underscore, regardless of the concrete response item type.
func TestOpenAIGatewayService_APIKeyPassthrough_PreservesPrefixedReasoningItemIDs(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	forwarded := upstream.lastBody
	require.Equal(t, "item_bad_reasoning", gjson.GetBytes(forwarded, "input.0.id").String(),
		"generic prefixed IDs must remain replayable")
...
func TestShouldStripOpenAIResponsesInputItemID_Reasoning(t *testing.T) {
	cases := []struct {
		...
		{"reasoning item_* id", "reasoning", "item_bad_reasoning", false},
		{"reasoning rs id", "reasoning", "rs_abc123", false},
		{"reasoning empty id", "reasoning", "", true},
		{"message msg id", "message", "msg_abc", false},
		{"message item id", "message", "item_x", false},
		{"function_call fc id", "function_call", "fc_abc", false},
		{"function_call item id", "function_call", "item_x", false},
		{"function_call ctc id", "function_call", "ctc_abc", false},
		{"custom tool ctc id", "custom_tool_call", "ctc_abc", false},
		{"custom tool fc id", "custom_tool_call", "fc_abc", false},
		{"tool search tsc id", "tool_search_call", "tsc_abc", false},
		{"tool search fc id", "tool_search_call", "fc_abc", false},
		{"web search ws id", "web_search_call", "ws_001", false},
		{"web search item id", "web_search_call", "item_001", false},
		{"custom output fc id", "custom_tool_call_output", "fc_001", false},
		{"custom output ctco id", "custom_tool_call_output", "ctco_001", false},
		{"unprefixed uuid", "message", "018f9e15-7a6a-7000-8000-000000000001", true},
	}
