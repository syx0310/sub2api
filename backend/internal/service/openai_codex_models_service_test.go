...
func TestNewConfiguredCodexModelDescriptorUsesProviderMetadataAndSafeFallback(t *testing.T) {
	...
	require.True(t, astra.SupportsExperimentalContext)
	...
	require.False(t, isOpenAIGPT6AstraModel("gpt-6"))
...
func TestMergeGroupConfiguredCodexModelsForcesSparkTextOnly(t *testing.T) {
	t.Parallel()
	...
	manifest := &OpenAIModelsResponse{Body: []byte(`{"models":[{"slug":"gpt-5.3-codex-spark","input_modalities":["text","image"],"supports_image_detail_original":true,"unknown":{"kept":true}}]}`)}
...
func TestBuildGroupConfiguredCodexModelsManifestKeepsSparkSupplementWithAdministratorConfiguration(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, configured)
	expected := append(openai.DefaultModelIDs(), "glm-5.3", "gpt-5.3-codex-spark")
	expected = FilterCodexModelIDsForGroup(dedupeAndSortModelIDs(expected), &Group{ID: groupID, Platform: PlatformOpenAI})
	require.ElementsMatch(t, expected, codexManifestModelSlugs(t, manifest.Body))
}
...
func TestObservedGPT6AstraCatalogSupplementsGroupManifest(t *testing.T) {
	...
	accounts := []Account{{
		...
		Extra: map[string]any{UpstreamModelMetadataExtraKey: UpstreamModelMetadataSnapshot{
			ModelIDsSyncedAt: now.Format(time.RFC3339),
...
func TestCompleteAPIKeyCodexModelsManifestForClientMarksOnlyOfficialVisionGPTImageInput(t *testing.T) {
	...
	svc := &OpenAIGatewayService{}
	manifest := &OpenAIModelsResponse{Body: []byte(`{"models":[{"slug":"gpt-6-astra"},{"slug":"gpt-5.6-sol"},{"slug":"gpt-4o"},{"slug":"gpt-5.3-codex-spark","input_modalities":["text","image"],"supports_image_detail_original":true},{"slug":"gpt-3.5-turbo"},{"slug":"gpt-4"}]}`)}
	...
	for _, slug := range []string{"gpt-5.3-codex-spark", "gpt-3.5-turbo", "gpt-4"} {
