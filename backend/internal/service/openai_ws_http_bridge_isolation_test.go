...
func TestOpenAIWSHTTPBridgeSessionIsolationAcrossSameSessionHash(t *testing.T) {
	for _, mode := range []string{OpenAIWSIngressModeHTTPBridge, OpenAIWSIngressModeCtxPool} {
		t.Run(mode, func(t *testing.T) { testOpenAIWSHTTPBridgeConnectionIsolation(t, mode) })
	}
}

func testOpenAIWSHTTPBridgeConnectionIsolation(t *testing.T, mode string) {
	...
	seedHash := openAIWSThreadStateHash(newContext(httptest.NewRequest(http.MethodGet, "/v1/responses", nil)), nil, account)
