...
func TestAnthropicToResponses_ThinkingWithGPTSignatureBecomesReasoningItem(t *testing.T) {
	...
	resp, err := AnthropicToResponsesForProvider(req, "openai")
	require.NoError(t, err)
...
func TestAnthropicToResponses_ThinkingWithGrokSignatureBecomesReasoning(t *testing.T) {
	req := &AnthropicRequest{
		Model:     "grok-4.5",
		MaxTokens: 1024,
		Messages: []AnthropicMessage{
	...
	resp, err := AnthropicToResponsesForProvider(req, "grok")
	require.NoError(t, err)
