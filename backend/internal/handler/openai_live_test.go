...
func TestLiveCallIdentityPreservesExplicitClientSessionID(t *testing.T) {
	gin.SetMode(gin.TestMode)
	request := httptest.NewRequest(http.MethodPost, "/v1/live", nil)
	// Codex realtime_conversation sends the requested realtime session in this
	// header for both Frameless Bidi and the other realtime transports.
	request.Header.Set("x-session-id", "live-session-123")
	context, _ := gin.CreateTestContext(httptest.NewRecorder())
	context.Request = request
	...
	require.Equal(t, "live-session-123", identity.SessionID)
}
