...
// A bare abnormal close comes from the downstream reader when no explicit
// ingress stage wrapper is available. It is not a clean client ending, but it
// still must not be charged to the selected upstream account; upstream close
// failures are wrapped with read_upstream/write_upstream before attribution.
func TestOpenAIWSIngressEndedByClient_BareAbnormalClosureIsNotChargedToAccount(t *testing.T) {
