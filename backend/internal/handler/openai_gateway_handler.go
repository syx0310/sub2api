...
func shouldReportOpenAIWSProxyAccountFailure(err error) bool {
	return err != nil &&
		...
		service.ShouldReportOpenAIWSAccountScheduleFailure(err)
}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, reqModel, err != nil, cyberBlockKeyHTTP, clientRequestedUsageFields(c, channelMapping, reqModel, ""), service.HashUsageRequestPayload(body), requestBodyBytes, responseBodyBytes)
		...
		clientDeliveryErr := err != nil && service.IsOpenAIWSClientDeliveryError(err)
		if clientDeliveryErr {
			reqLog.Info("openai.client_delivery_failed_after_upstream_completion",
				zap.Int64("account_id", account.ID),
				zap.Error(err),
			)
		}
		if err != nil && !clientDeliveryErr {
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		responseBodyBytes := usageResponseBodyBytesFromGin(c)
		h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, reqModel, err != nil, cyberBlockKeyMsg, clientRequestedUsageFields(c, channelMappingMsg, reqModel, ""), service.HashUsageRequestPayload(body), requestBodyBytes, responseBodyBytes)
		forwardDurationMs := time.Since(forwardStart).Milliseconds()
		upstreamLatencyMs, _ := getContextInt64(c, service.OpsUpstreamLatencyMsKey)
		responseLatencyMs := forwardDurationMs
		if upstreamLatencyMs > 0 && forwardDurationMs > upstreamLatencyMs {
...
// BeforeTurn 在每个 turn 执行利润复核时写入兜底时刻；正常计费优先使用
// TurnStarted 捕获的客户端 response.create 接受时刻。turn 在连接内串行推进，
// 互斥锁只为跨用量提交 goroutine 的读取安全。
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	// Codex sends only an incremental delta when previous_response_id is set.
	// Even a delta without tool output is not a complete conversation, so it
	// must never be made movable by deleting its response anchor.
	previousResponseCanMove := previousResponseID == ""
	reqLog = reqLog.With(
	...
	wsAttemptIsRebuiltCurrentTurn := false
	wsAttemptStartedAt := firstTurnStartedAt
	...
	for {
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
				...
				turnUpstreamModel := ""
				if result != nil {
					if model := strings.TrimSpace(result.Model); model != "" && turnRequestPayloadHash == "" {
						turnRequestedModel = model
					}
					turnUpstreamModel = strings.TrimSpace(result.UpstreamModel)
					if result.RequestBodyBytes != nil {
						turnRequestBodyBytes = result.RequestBodyBytes
					}
				}
				...
				if turnUpstreamModel == "" {
					mappedModel := strings.TrimSpace(turnMapping.MappedModel)
					if mappedModel == "" {
						mappedModel = turnRequestedModel
					}
					turnUpstreamModel = strings.TrimSpace(account.GetMappedModel(mappedModel))
					if turnUpstreamModel == "" {
						turnUpstreamModel = mappedModel
					}
				}
				turnUsageFields := turnMapping.ToUsageFields(turnRequestedModel, turnUpstreamModel)
				var turnResponseBodyBytes *int64
				if result != nil {
					turnResponseBodyBytes = result.ResponseBodyBytes
				}
				h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, turnRequestedModel, turnErr != nil, cyberBlockKey, turnUsageFields, turnRequestPayloadHash, turnRequestBodyBytes, turnResponseBodyBytes)
				if service.GetOpsCyberPolicy(c) != nil {
				...
				if turnErr != nil {
					if result == nil || result.ImageCount <= 0 {
						var failoverErr *service.UpstreamFailoverError
						if result == nil &&
							!errors.As(turnErr, &failoverErr) &&
							shouldReportOpenAIWSProxyAccountFailure(turnErr) {
							turnScheduleResultReported.Store(true)
							h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, turnUpstreamModel, false, nil)
						}
						return
					}
				...
				quotaPlatform := service.QuotaPlatform(c.Request.Context(), apiKey)
				sessionID := service.ExtractClientSessionID(c)
				turnRecordPricingAt := turnStart
				if turnRecordPricingAt.IsZero() {
					turnRecordPricingAt = turnPricing.currentOr(time.Time{})
				}
				cyberBlocked := service.GetOpsCyberPolicy(c) != nil
		...
		// A Codex frame carrying previous_response_id contains only the incremental
		// input. Do not strip the anchor on an initial cross-account selection: the
		// handler does not own the missing history. Current-turn failover below uses
		// a separately reconstructed full payload and is therefore safe to move.
		attemptHooks := hooks
		if wsAttemptIsRebuiltCurrentTurn {
			attemptBodyBytes := usageBodyBytesPtr(len(wsAttemptMessage))
			attemptModel := strings.TrimSpace(gjson.GetBytes(wsAttemptMessage, "model").String())
			if attemptModel == "" {
			...
			attemptStartedAt := wsAttemptStartedAt
			clonedHooks.BeforeTurn = func(turn int) error {
				if beforeTurn != nil {
					if err := beforeTurn(turn); err != nil {
						return err
					}
				}
				if turn == 1 && !attemptStartedAt.IsZero() {
					turnPricing.freeze(attemptStartedAt)
				}
				return nil
			}
			attemptHooks = &clonedHooks
		}

		if err := h.gatewayService.ProxyResponsesWebSocketFromClient(ctx, c, wsConn, account, token, wsAttemptMessage, attemptHooks); err != nil {
			var failoverErr *service.UpstreamFailoverError
			if errors.As(err, &failoverErr) {
				...
				if retryCurrentTurn {
					wsAttemptIsRebuiltCurrentTurn = true
					wsAttemptStartedAt = turnPricing.currentOr(time.Now())
					previousResponseID = ""
			...
			clientDeliveryErr := service.IsOpenAIWSClientDeliveryError(err)
			if !turnScheduleResultReported.Load() && shouldReportOpenAIWSProxyAccountFailure(err) {
				...
				if snapshot := turnChannelMapping.Load(); snapshot != nil {
					if mappedModel := strings.TrimSpace(snapshot.mapping.MappedModel); mappedModel != "" {
						scheduleModel = account.GetMappedModel(mappedModel)
					}
				}
				h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, scheduleModel, false, nil)
			}
			if clientDeliveryErr {
				reqLog.Info("openai.websocket_client_delivery_failed_after_drain",
					zap.Int64("account_id", account.ID),
					zap.Error(err),
				)
				return
...
func (h *OpenAIGatewayHandler) recordCyberPolicyIfMarked(c *gin.Context, apiKey *service.APIKey, account *service.Account, subscription *service.UserSubscription, model string, forwardErrored bool, cyberBlockKey string, channelFields service.ChannelUsageFields, requestPayloadHash string, requestBodyBytes, responseBodyBytes *int64) {
