...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, reqModel, err != nil, cyberBlockBodyHTTP, clientRequestedUsageFields(c, channelMapping, reqModel, ""), service.HashUsageRequestPayload(body), requestBodyBytes, responseBodyBytes)
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		responseBodyBytes := usageResponseBodyBytesFromGin(c)
		h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, reqModel, err != nil, cyberBlockBodyMsg, clientRequestedUsageFields(c, channelMappingMsg, reqModel, ""), service.HashUsageRequestPayload(body), requestBodyBytes, responseBodyBytes)
		forwardDurationMs := time.Since(forwardStart).Milliseconds()
		upstreamLatencyMs, _ := getContextInt64(c, service.OpsUpstreamLatencyMsKey)
		responseLatencyMs := forwardDurationMs
		if upstreamLatencyMs > 0 && forwardDurationMs > upstreamLatencyMs {
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	for {
				...
				if turnErr != nil {
					if result == nil || result.ImageCount <= 0 {
						...
						if result == nil &&
							...
							h.gatewayService.ReportOpenAIAccountScheduleResult(account, turnUpstreamModel, false, nil, turnErr)
						}
						return
					}
		...
		attemptHooksForMessage := func() *service.OpenAIWSIngressHooks {
		...
		for {
			...
			if !turnScheduleResultReported.Load() && shouldReportOpenAIWSProxyAccountFailure(err) {
				...
				h.gatewayService.ReportOpenAIAccountScheduleResult(account, scheduleModel, false, nil, err)
			}
			if clientDeliveryErr {
