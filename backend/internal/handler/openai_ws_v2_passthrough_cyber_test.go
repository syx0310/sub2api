...
func TestOpenAIResponsesWebSocketV2PassthroughNonCyberTurnAllowsFollowup(t *testing.T) {
	...
	upstreamServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		firstCreated := []byte(`{"type":"response.created","response":{"id":"resp_non_cyber_handler_turn_1","model":"gpt-5.1"}}`)
		writeCtx, cancelWrite := context.WithTimeout(r.Context(), 3*time.Second)
		err = conn.Write(writeCtx, coderws.MessageText, firstCreated)
		cancelWrite()
		require.NoError(t, err)
		...
		secondCreated := []byte(`{"type":"response.created","response":{"id":"resp_non_cyber_handler_turn_2","model":"gpt-5.1"}}`)
		writeCtx, cancelWrite = context.WithTimeout(r.Context(), 3*time.Second)
		err = conn.Write(writeCtx, coderws.MessageText, secondCreated)
		cancelWrite()
		require.NoError(t, err)
	...
	readCtx, cancelRead := context.WithTimeout(context.Background(), 3*time.Second)
	_, firstCreatedEvent, err := harness.clientConn.Read(readCtx)
	cancelRead()
	require.NoError(t, err)
	require.Equal(t, "response.created", gjson.GetBytes(firstCreatedEvent, "type").String())
	...
	readCtx, cancelRead = context.WithTimeout(context.Background(), 3*time.Second)
	_, secondCreatedEvent, err := harness.clientConn.Read(readCtx)
	cancelRead()
	require.NoError(t, err)
	require.Equal(t, "response.created", gjson.GetBytes(secondCreatedEvent, "type").String())
