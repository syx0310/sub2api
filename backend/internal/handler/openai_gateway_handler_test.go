...
func TestShouldReportOpenAIWSProxyAccountFailure(t *testing.T) {
	...
	t.Run("local policy close does not penalize account", func(t *testing.T) {
...
func TestOpenAIResponsesWebSocket_FailoverPreservesPreviousResponseIDWhenStickyMisses(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	secondUpstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		writeCtx, cancelWrite := context.WithTimeout(r.Context(), 3*time.Second)
		_ = conn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.completed","response":{"id":"resp_ws_failover_tool_ok","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`))
		cancelWrite()
		_ = conn.Close(coderws.StatusNormalClosure, "done")
	}))
	defer secondUpstream.Close()
	...
	accounts := []service.Account{
		{
			ID:          9912,
			Name:        "openai-ws-rate-limited-prev",
			Platform:    service.PlatformOpenAI,
			Type:        service.AccountTypeAPIKey,
			Status:      service.StatusActive,
			Schedulable: true,
	...
	firstMessage := `{"type":"response.create","model":"gpt-5.1","stream":false,"previous_response_id":"resp_prev_tool_failover","input":[{"type":"function_call","call_id":"call_failover_1","name":"shell","arguments":"{}"},{"type":"function_call_output","call_id":"call_failover_1","output":"ok"}]}`
	writeCtx, cancelWrite := context.WithTimeout(context.Background(), 3*time.Second)
	...
	require.NoError(t, err)
	require.Equal(t, "response.completed", gjson.GetBytes(event, "type").String())
	require.Equal(t, "resp_ws_failover_tool_ok", gjson.GetBytes(event, "response.id").String())
	...
	require.Equal(t, "resp_prev_tool_failover", gjson.GetBytes(firstPayload, "previous_response_id").String(), "代理不能把增量请求误当成完整历史并删除 previous_response_id")
	require.Equal(t, "resp_prev_tool_failover", gjson.GetBytes(secondPayload, "previous_response_id").String(), "跨账号 failover 必须原样重放客户端请求；状态不可用时由客户端重连并发送完整历史")
	require.Equal(t, "function_call", gjson.GetBytes(secondPayload, "input.0.type").String())
	require.Equal(t, "function_call_output", gjson.GetBytes(secondPayload, "input.1.type").String())
	require.Equal(t, []int64{int64(9912)}, accountRepo.rateLimitedIDs)
}
...
func runOpenAIResponsesWebSocketUsageLogCase(t *testing.T, tc openAIResponsesWSUsageLogCase) openAIResponsesWSUsageLogResult {
	...
	upstreamServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for turn := 1; turn <= turnCount; turn++ {
			...
			upstreamModel := gjson.GetBytes(payload, "model").String()
			for _, response := range []string{
				...
				fmt.Sprintf(
					`{"type":"response.completed","response":{"id":%q,"model":%q,"usage":{"input_tokens":2,"output_tokens":1}}}`,
					responseID,
					upstreamModel,
				),
			} {
	...
	readCompleted := func() {
		for _, wantType := range []string{"response.created", "response.completed"} {
			...
			if wantType == "response.completed" {
