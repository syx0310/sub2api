...
type GatewayConfig struct {
	...
	// OpenAI 上游传输层错误是否触发 failover/临时下线（默认关闭，避免改变旧 502 语义）
	OpenAITransportErrorFailoverEnabled bool `mapstructure:"openai_transport_error_failover_enabled"`
...
type GatewayOpenAIWSConfig struct {
	...
	// IngressPreviousResponseRecoveryEnabled is retained for configuration compatibility only.
	// Deprecated: ingress forwards previous_response_not_found to the Codex client,
	// which owns the complete history needed for a safe retry.
...
func NormalizeRunMode(value string) string {
	...
	switch normalized {
	case RunModeStandard, RunModeSimple, RunModeRelay:
		return normalized
	default:
...
func setDefaults() {
	...
	viper.SetDefault("gateway.openai_transport_error_failover_enabled", false)
