...
var modelStatsSQLColumns = []string{
	"model", "requests", "input_tokens", "output_tokens",
	"cache_creation_tokens", "cache_read_tokens", "total_tokens",
	"cost", "actual_cost", "input_actual_cost", "output_actual_cost",
	"cache_creation_actual_cost", "cache_read_actual_cost", "other_actual_cost", "account_cost",
}

var usageStatsSQLColumns = []string{
	...
	"cost", "actual_cost", "input_actual_cost", "output_actual_cost",
	"cache_creation_actual_cost", "cache_read_actual_cost", "other_actual_cost",
	"account_cost", "avg_duration_ms",
}

func usageLogInsertSQLRegexp() string {
	return fmt.Sprintf(`INSERT INTO usage_logs.*\$%d\s*\)`, len(usageLogInsertArgTypes))
}
...
func TestUsageLogRepositoryCreateSyncRequestTypeAndLegacyFields(t *testing.T) {
	...
	mock.ExpectQuery(usageLogInsertSQLRegexp()).
		WithArgs(
			log.UserID,
			log.APIKeyID,
			log.AccountID,
...
func TestExecUsageLogInsertNoResult_PersistsRequestedModel(t *testing.T) {
	...
	mock.ExpectExec(usageLogInsertSQLRegexp()).
...
func TestUsageLogRepositoryGetModelStatsWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("AND \\(request_type = \\$3 OR \\(request_type = 0 AND openai_ws_mode = TRUE\\)\\)").
		WithArgs(start, end, requestType).
		WillReturnRows(sqlmock.NewRows(modelStatsSQLColumns))
...
func TestUsageLogRepositoryGetUserModelStatsUsesRequestedModel(t *testing.T) {
	...
	mock.ExpectQuery("(?s)SELECT\\s+COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) as model,.*WHERE created_at >= \\$1 AND created_at < \\$2\\s+AND user_id = \\$3.*GROUP BY COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) ORDER BY total_tokens DESC").
		...
		WillReturnRows(sqlmock.NewRows(modelStatsSQLColumns).
			AddRow("gpt-5.5", int64(2), int64(10), int64(20), int64(0), int64(0), int64(30), 0.1, 0.08, 0.04, 0.02, 0.01, 0.005, 0.005, 0.07))
...
func TestUsageLogRepositoryGetStatsWithFiltersRequestedModelSource(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs\\s+WHERE COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\$1.*GROUP BY GROUPING SETS").
		WithArgs("gpt-5").
		WillReturnRows(sqlmock.NewRows(usageStatsSQLColumns).
			AddRow(1, 1, nil, nil, int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 0.4, 0.3, 0.2, 0.1, 0.0, 1.2, 20.0).
			AddRow(0, 1, "/v1/responses", nil, int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 0.4, 0.3, 0.2, 0.1, 0.0, 1.2, 20.0).
			AddRow(1, 0, nil, "/v1/responses", int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 0.4, 0.3, 0.2, 0.1, 0.0, 1.2, 20.0).
			AddRow(0, 0, "/v1/responses", "/v1/responses", int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 0.4, 0.3, 0.2, 0.1, 0.0, 1.2, 20.0))
...
func TestUsageLogRepositoryGetModelStatsAccountCostColumn(t *testing.T) {
	...
	mock.ExpectQuery("FROM usage_logs").
		WithArgs(start, end).
		WillReturnRows(sqlmock.NewRows(modelStatsSQLColumns).
			AddRow("claude-opus-4-6", int64(10), int64(100), int64(200), int64(5), int64(3), int64(308), 2.5, 2.0, 1.0, 0.5, 0.2, 0.1, 0.2, 1.8).
			AddRow("claude-sonnet-4-6", int64(5), int64(50), int64(100), int64(0), int64(0), int64(150), 1.0, 0.8, 0.4, 0.2, 0.0, 0.0, 0.2, 0.7))
...
func TestUsageLogRepositoryGetModelStatsWithUsageFiltersAppliesRequestedModelFilter(t *testing.T) {
	...
	mock.ExpectQuery("AND COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\$3").
		WithArgs(start, end, "gpt-5").
		WillReturnRows(sqlmock.NewRows(modelStatsSQLColumns).
			AddRow("gpt-5", int64(1), int64(10), int64(20), int64(0), int64(0), int64(30), 0.1, 0.08, 0.04, 0.02, 0.0, 0.0, 0.02, 0.07))
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs.*GROUP BY GROUPING SETS").
		WillReturnRows(sqlmock.NewRows(usageStatsSQLColumns).
			AddRow(1, 1, nil, nil, int64(50), int64(1000), int64(2000), int64(60), int64(40), 15.0, 12.5, 6.0, 3.0, 1.0, 0.5, 2.0, 11.0, 100.0))
