...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	clear()
	_, err = integrationDB.ExecContext(ctx, "UPDATE groups SET max_reasoning_effort = 'high' WHERE id = $1", group.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "reasoning-effort ceiling changes must enqueue bound keys")
	clear()
	_, err = integrationDB.ExecContext(ctx, `
		UPDATE groups
		SET reasoning_effort_mappings = '[{"from":"xhigh","to":"high"}]'::jsonb
		WHERE id = $1`, group.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "reasoning-effort mapping changes must enqueue bound keys")
	clear()
