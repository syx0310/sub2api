...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	`, buildWhere(conditions), actualCosts.Input, actualCosts.Output, actualCosts.CacheCreation, actualCosts.CacheRead, actualCosts.Other)
	...
	for rows.Next() {
		var (
			...
			cost, actualCost, inputActualCost, outputActualCost                  float64
			cacheCreationActualCost, cacheReadActualCost, otherActualCost        float64
			accountCost, averageDurationMs                                       float64
		)
		if err := rows.Scan(
			&inboundGrouped,
			&upstreamGrouped,
		...
		case inboundGrouped == 1 && upstreamGrouped == 1:
			...
			stats.InputActualCost = inputActualCost
			stats.OutputActualCost = outputActualCost
			stats.CacheCreationActualCost = cacheCreationActualCost
			stats.CacheReadActualCost = cacheReadActualCost
			stats.OtherActualCost = otherActualCost
