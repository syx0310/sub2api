...
func TestMigration227BackfillsForkDefaultDeviceAccounts(t *testing.T) {
	tx := testTx(t)
	ctx := context.Background()
	migrationSQL, err := dbmigrations.FS.ReadFile("227_backfill_default_codex_fingerprint_seed.sql")
	require.NoError(t, err)

	insertAccount := func(name, accountType, extra string) int64 {
		t.Helper()
		var id int64
		require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO accounts (name, platform, type, extra)
...
RETURNING id
`, name, accountType, extra).Scan(&id))
		return id
	}

	missingModeID := insertAccount("migration-227-missing-mode", service.AccountTypeOAuth, `{}`)
	invalidModeID := insertAccount("migration-227-invalid-mode", service.AccountTypeOAuth, `{"codex_fingerprint_mode":"legacy","codex_fingerprint_seed":"BAD"}`)
	validSeedID := insertAccount("migration-227-valid-seed", service.AccountTypeOAuth, `{"codex_fingerprint_seed":"11111111-1111-4111-8111-111111111111"}`)
	offID := insertAccount("migration-227-off", service.AccountTypeOAuth, `{"codex_fingerprint_mode":"off"}`)
	apiKeyID := insertAccount("migration-227-apikey", service.AccountTypeAPIKey, `{}`)

	_, err = tx.ExecContext(ctx, string(migrationSQL))
	require.NoError(t, err)

	seedsAfterFirst := map[int64]string{}
	for _, id := range []int64{missingModeID, invalidModeID, validSeedID} {
		var seed string
		require.NoError(t, tx.QueryRowContext(ctx, `SELECT extra->>'codex_fingerprint_seed' FROM accounts WHERE id = $1`, id).Scan(&seed))
		requireCanonicalUUIDString(t, seed)
		seedsAfterFirst[id] = seed
	}
	require.Equal(t, "11111111-1111-4111-8111-111111111111", seedsAfterFirst[validSeedID])
