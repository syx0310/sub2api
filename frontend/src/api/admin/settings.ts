...
const createAuthSourceDefaultsValue = (): AuthSourceDefaultsValue => ({
...
export function appendAuthSourceDefaultsToUpdateRequest(
  ...
  for (const source of AUTH_SOURCE_TYPES) {
    const current = authSourceDefaults[source] ?? createAuthSourceDefaultsValue();
...
export interface SystemSettings {
  ...
  openai_transport_error_failover_enabled: boolean;
...
export interface WebSearchProviderConfig {
  type: "brave" | "tavily" | "tavily_hikari";
  api_key: string;
