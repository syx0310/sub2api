...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('admin.usage.requestId'), t('admin.usage.upstreamRequestId'), t('usage.userAgent'), t('admin.usage.ipAddress'), t('admin.usage.bodySize')
    ]
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.request_id || '', log.upstream_request_id || '', log.user_agent || '', log.ip_address || '', formatExportBytes(log)
      ])
      if (rows.length) {
